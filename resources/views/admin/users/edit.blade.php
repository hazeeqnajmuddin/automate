...
<div class="container mx-auto p-4 md:p-8">
    <div class="max-w-2xl mx-auto">
        ...
        <h1 class="text-3xl font-bold border-b-2 border-gray-800 pb-2 mb-8">Edit User: {{ $user->username }}</h1>
        ...
        <div class="bg-white p-6 rounded-lg shadow-md">
            <form action="{{ route('admin.users.update', $user) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div class="flex items-center space-x-6">
                    <div class="shrink-0">
                        {{-- UPDATED: Added an ID to the image tag for the preview script --}}
                        <img id="profile-pic-preview" class="h-20 w-20 object-cover rounded-full" src="{{ $user->profile_pic_path ? asset('storage/' . $user->profile_pic_path) : 'https://placehold.co/80x80/e2e8f0/e2e8f0' }}" alt="Current profile photo" />
                    </div>
                    <label class="block">
                        <span class="sr-only">Choose profile photo</span>
                        {{-- UPDATED: Added an ID to the file input --}}
                ...
                <div>
                    ...
                    <input type="text" id="full_name" name="full_name" value="{{ old('full_name', $user->full_name) }}" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required>
                ...
                <div>
                    ...
                    <input type="text" id="username" name="username" value="{{ old('username', $user->username) }}" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required>
                ...
                <div>
                    ...
                    <input type="email" id="user_email" name="user_email" value="{{ old('user_email', $user->user_email) }}" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required>
    ...
    @error('phone_number') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                ...
                <div>
                    ...
                    <select id="user_role" name="user_role" class="mt-1 block w-full px-3 py-2 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required>
                        <option value="car_owner" {{ old('user_role', $user->user_role) == 'car_owner' ? 'selected' : '' }}>Car Owner</option>
                ...
                <div>
                    ...
                    <input type="password" id="password_confirmation" name="password_confirmation" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm">
                </div>
