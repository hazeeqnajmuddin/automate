...
@section('title', "Garage: " . $user->username)
        ...
        <header class="flex justify-between items-center mb-12">
            <div>
                ...
                <h1 class="text-5xl font-black text-gray-900 leading-tight tracking-tighter uppercase italic">
                    User <span class="text-indigo-600">Garage</span>
                </h1>
                <p class="text-xl text-gray-500 mt-2 font-medium italic">Viewing vehicles registered to {{ $user->full_name }}</p>
            </div>
            
            <a href="{{ route('admin.users.show', $user) }}" class="group inline-flex items-center px-6 py-3 bg-gray-100 text-gray-800 rounded-xl font-black text-xs uppercase tracking-widest hover:bg-gray-200 transition-all">
                ...
                <div class="bg-slate-900 p-12 rounded-[3rem] shadow-2xl flex flex-col items-center justify-center w-full border-4 border-indigo-500/30">
                    <div class="relative mb-8">
                        @if ($user->profile_pic_path)
                            <img src="{{ asset('storage/' . $user->profile_pic_path) }}" alt="Profile Picture" class="h-32 w-32 rounded-3xl object-cover border-4 border-white shadow-2xl rotate-3">
                        @else
                        ...
                        @endif
                        <div class="absolute -bottom-2 -right-2 bg-emerald-500 p-2 rounded-xl shadow-lg border-4 border-slate-900">
                    ...
                    <div class="text-center">
                        <h2 class="text-2xl font-black text-white tracking-tighter uppercase italic">{{ $user->username }}</h2>
                        <p class="text-indigo-400 font-bold mt-1 uppercase tracking-widest text-[10px]">Verified Owner</p>
                    </div>
                    ...
                    <div class="mt-10 w-full space-y-4">
                        ...
                        <div class="bg-white/5 p-4 rounded-2xl border border-white/10 flex justify-between items-center">
                            <span class="text-[9px] font-black text-gray-500 uppercase tracking-widest">Last Entry</span>
                            <span class="text-indigo-400 font-black italic text-xs">{{ $cars->count() > 0 ? $cars->sortByDesc('created_at')->first()->created_at->format('M Y') : 'N/A' }}</span>
                        </div>
                    </div>

                    <p class="text-[9px] text-gray-600 mt-8 font-bold uppercase tracking-[0.2em] text-center">System Audit Log Active</p>
                </div>
            </div>
            ...
            <div class="w-full lg:w-2/3 flex flex-col space-y-8">
                @forelse ($cars as $car)
                    <div class="group relative bg-white p-10 rounded-[2.5rem] border border-gray-100 shadow-xl shadow-gray-200/50 transition-all duration-500 hover:shadow-indigo-500/10 hover:-translate-y-2 border-l-8 border-l-indigo-600 overflow-hidden">
                        ...
                        <!-- Floating Badge -->
                        <div class="absolute top-6 right-8 text-[10px] font-black uppercase tracking-widest text-gray-300">
                            Modified {{ $car->updated_at->diffForHumans() }}
                        </div>

                        <div class="flex flex-col md:flex-row items-center gap-10">
                            {{-- Visual Image / Indicator --}}
                            ...
                            <div class="flex-1 text-center md:text-left">
                                ...
                                <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mt-8">
                                    ...
                                    <div>
                                        <p class="text-[9px] font-black text-gray-400 uppercase tracking-widest">Age</p>
                            ...
                            <div class="shrink-0 flex flex-col space-y-3 pt-6 md:pt-0">
                                <a href="{{ route('admin.users.car_details', ['user' => $user, 'car' => $car]) }}" class="inline-flex items-center justify-center px-8 py-3 bg-indigo-600 text-white rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-indigo-700 transition-all shadow-lg active:scale-95">
                                    View
                                </a>
                            </div>
                        </div>
                ...
                @empty
                    <div class="bg-slate-50 p-20 rounded-[2.5rem] border border-dashed border-gray-200 text-center">
                        ...
                        <h3 class="text-xl font-black text-gray-400 uppercase tracking-tighter italic">Garage Is Empty</h3>
                        <p class="text-gray-400 mt-2 font-medium">This user has not registered any vehicles to the platform.</p>
                    </div>
                @endforelse

                <div class="pt-8 text-center">
                    <p class="text-[10px] font-black text-gray-400 uppercase tracking-[0.3em]">End of Fleet Records</p>
                </div>
            </div>
        </div>
    </div>
